...
def main():
    # 设置参数
    wiki_embed_path = 'data/sgns.wiki.bigram'
    batch_size = 128
    nb_filters = 128
    dropout_rate = 0.3
    embed_size = 300
    ...
    train_dataset = gen_data('data/%s' % dataset_name, train_set, vocab, test_vocab, True, max_query_len, max_doc_len)
    print("Create training set successfully...")
    val_dataset = gen_data('data/%s' % dataset_name, val_set, vocab, test_vocab, True, max_query_len, max_doc_len)
    test_dataset = gen_data('data/%s' % dataset_name, test_set, vocab, test_vocab, False, max_query_len, max_doc_len)
